...
class OpenWeatherAPI(BasicFetcher):
            ...
            try:
                async with httpx.AsyncClient() as client:
                    for city in self._cities:
                        resp = await client.get(self._source_url, params={'appid': self._api_key, 'city': city})
                        resp = resp.json()
                        resp['source_provider'] = 'openweathermap'
                        output.append(resp)
    ...
    async def run(self) -> None:
        ...
        """
        await self.connect()
        asyncio.create_task(self.read_and_update_conf(0))
        while True:
            await self.publish()
